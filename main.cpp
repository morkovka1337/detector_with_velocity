...
#include "include/image_reader.hpp"
#include "include/pedestrian_tracker_demo.hpp"
...
int main_work(int argc, char **argv) {
    ...
    // auto delta_time = 2500; //  milliseconds
    auto velocity_freq = 30;
    ...
    std::cout << std::endl;
    std::unordered_map<int, cv::Rect> prev_bboxes {};
    uint64_t cur_time = 0;
    ...
    for (;;) {
        ...
        if (should_show) {
            ...
            for (const auto &detection : tracker->TrackedDetections()) {
                ...
                auto vel_text_point = detection.rect.br();
                if (frame_idx % velocity_freq == 0) {
                    if (prev_bboxes.find(detection.object_id) == prev_bboxes.end() )
                    {
                        prev_bboxes[detection.object_id] = detection.rect;
                    }
                    else {
                        auto prev_bb = prev_bboxes.at(detection.object_id);
                        auto prev_bb_x = static_cast<double>(prev_bb.x) + static_cast<double>(prev_bb.width) / 2;
                        auto prev_bb_y = static_cast<double>(prev_bb.y) + static_cast<double>(prev_bb.height) / 2;
                        auto cur_bb = detection.rect;
                        auto cur_bb_x = static_cast<double>(cur_bb.x) + static_cast<double>(cur_bb.width) / 2;
                        auto cur_bb_y = static_cast<double>(cur_bb.y) + static_cast<double>(cur_bb.height) / 2;
                        auto delta_time = cur_timestamp - cur_time;
                        cur_velocity = sqrt((cur_bb_x - prev_bb_x) * (cur_bb_x - prev_bb_x) + 
                                    (cur_bb_y - prev_bb_y) * (cur_bb_y - prev_bb_y)) / 
                                        static_cast<double>(delta_time);
                        
                       
                        
                        // std::string text = "Velocity: " + std::to_string(cur_velocity);
                        // cv::putText(frame, text, vel_text_point, cv::FONT_HERSHEY_COMPLEX,
                        //             1.0, cv::Scalar(0, 255, 0), 3);
                        cur_time = cur_timestamp;
                        prev_bboxes[detection.object_id] = cur_bb;
                    }
                }
                text = "Velocity: " + std::to_string(cur_velocity);
                cv::putText(frame, text, vel_text_point, cv::FONT_HERSHEY_COMPLEX,
                                    1.0, cv::Scalar(0, 255, 0), 3);
